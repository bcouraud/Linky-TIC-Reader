...
const char* ap_ssid = "WattyWiFi"; //Access Point SSID
...
uint16_t url_port = 443;  // default HTTPS
...
int Powermax = 20000; // to be read from Linky, and stored in EEPROM
long int t1 = 0;
long int t2=0;
float voltageThreshold = 5.85; //190; //207; // voltage limit to startup the communication
float voltageThreshold2 = 5.1; //170; // voltage limit to startup the process
int epoch_time = 0;
...
String readFile(const char* path) {
  ...
  return content;
   ...
   void setup_wifi() { //String host_parameter) {
 ...
 yearstringend = String(year(epoch_time));
  ...
  if (prov == "unica") {
        ...
        httpRequestData = "linkySensor,sensor_id=" + sensor +
         ...
         if (!client.connect(host_url, url_port)) {
            //Connection failed
            return;
         }
...
/*
      ...
      // Check for success (HTTP/1.1 200 OK or similar)
      if (response.indexOf("200 OK") != -1) {
      Serial.println(" Server replied: 200 OK");
      return true;
      } else {
      Serial.println("Server error or unexpected reply:");
...
*/
         ...
         httpRequestData = "{\"assetID\": \"" + asset + "\",\"startedAt\": \"" +
             ...
             if (client.connect(host_url.c_str(), 443)) {
                     ...
                     client.println("User-Agent: WattyLinkyUniCA");
...
void readLinky1200(){
     ...
     if (indice>=5 )//&& (caracter == ' ' || isdigit(caracter)==false)) // if we receive a space, the power information is over. could be more robust by considering anything above 9
      {
        ...
        finished_recording=true;
                 if (debug==1){
        Serial.print("got a puissance_int:");
        Serial.println(puissance_int);}
         for (int i = 0; i < sizeof(puissance)/sizeof(puissance[0]); i++){
          puissance[i] = 0;}
 ...
 void setup() {
   ...
   pinMode(5, OUTPUT); // communication with PIC
 ...
 // Serial.println("Hello");
   pinMode(4, INPUT); // reading Voltage (digital) on D2 from the USB
  int connection_USB = digitalRead(4);  // will be 0 (LOW) or 1 (HIGH)
...
*/
                     ...
                     server.on("/save",HTTP_POST,handle_submitted);
   ...
   while(1){
       ...
       // We are connected to the Linky
                     digitalWrite(5, HIGH); // tells PIC that ESP is running  5
                     ...
                     real_voltage = (rawADC/1024.0)*3.1*2 ;   //5.24V --> 181  or 5.06V -->174  or 4.90 -->170 or 4.18 --> 147
  ...
  wifi_pass = readFile("/pass.txt");
                                 ...
                                 // Use EEPROM values
                                 ssid_Wifi     = wifi_ssid.c_str();
                                 password_Wifi = wifi_pass.c_str();
                                 } 
                                 ...
                                 rawADC = analogRead(A0); // we read voltage
                                  real_voltage = (rawADC/1024.0)*3.1*2 ;
                                          if (debug==1){
                                 Serial.println("Starting Reading loop");
                                 rawADC = 255;}

                                 timeoutReadLinky = 0;
                                 while(finished_recording == false && timeoutReadLinky<8000){  
                                 ...
                                 if(debug==1 || (real_voltage>voltageThreshold && finished_recording && !dataEEPROM.sent_last_time &&  counter_moyenne>=1)) {  // if we have enough voltage, we send the data and reinitialise the values
                           ...
                           delay(50);
                           digitalWrite(LED_BUILTIN, HIGH);
                           digitalWrite(5, LOW); // ready to be turned off by the PIC 
                           if (debug==1){
...
void handle_submitted()
{
         ...
         String message;
         Serial.println(server.arg("host"));
         Serial.println(server.arg("url"));
         Serial.println(server.arg("ssid"));
         Serial.println(server.arg("asset_id"));
         Serial.println(server.arg("body"));
         Serial.println(server.arg("method"));
         
    if (server.hasArg("ssid")) {
         writeFile("/ssid.txt", server.arg("ssid"));
    } else {
      message  = "<h3>Impossible de se connecter au WiFi.</h3>";
      message += "<p>Veuillez revenir sur la page precedente et selectionnez un reseau Wifi existant.</p>";
    ...
    if (server.hasArg("pass")) {
      writeFile("/pass.txt", server.arg("pass"));
       } else {
      message  = "<h3>Impossible de se connecter au WiFi car il n'y a pas de mot de passe.</h3>";
      message += "<p>Veuillez revenir sur la page precedente et entrez le mot de passe de votre un reseau Wifi.</p>";
               String fullPage = header + message + footer;
               server.send(200, "text/html", fullPage);
            return;
    }
    ...
    if (server.hasArg("provider")) {
      writeFile("/provider.txt", server.arg("provider"));
       } else {
      message  = "<h3>Vous n'avez pas choisi de fournisseur de service (serveur) du Watty.</h3>";
      message += "<p>Veuillez revenir sur la page precedente et re-selectionnez le wifi, mot de passe, et n'oubliez pas de choisir un fournisseur de service (serveur) du Watty.</p>";
               String fullPage = header + message + footer;
               server.send(200, "text/html", fullPage);
            return;
    }
    ...
    if (server.arg("provider") == "unica") {
      ...
      //         String url = "/nodes/imredd/energyconso/linky";
      writeFile("/url.txt", "/nodes/imredd/energyconso/linky");
      writeFile("/sensor_id.txt", server.arg("sensor_idunica"));
    }
    else if (server.arg("provider") == "chargeangels") {
      writeFile("/host.txt", server.arg("hostca"));// "imredd.charge-angels.com");
      writeFile("/url.txt", server.arg("urlca"));//"/v1/api/assets/" + server.arg("asset_id") + "/consumptions");
            Serial.print("wrote the followng Host: ");
      Serial.println(server.arg("hostca"));
                  Serial.print("wrote the followng url: ");
      Serial.println(server.arg("urlca"));



      writeFile("/asset_id.txt", server.arg("asset_idca"));
      writeFile("/token.txt", server.arg("tokenca"));
    }
    else if (server.arg("provider") == "other") {
      Serial.println("received data for the other provider");
      writeFile("/host.txt", server.arg("hostother"));
      Serial.print("wrote the followng Host: ");
      Serial.println(server.arg("hostother"));
      
      writeFile("/url.txt", server.arg("urlother"));
            Serial.print("wrote the followng url: ");
      Serial.println(server.arg("urlother"));

      writeFile("/port.txt", server.arg("portother"));
      writeFile("/method.txt", server.arg("methodother"));
      writeFile("/ctype.txt", server.arg("ctypeother"));
         //   writeFile("/token.txt", server.arg("tokenother"));

      writeFile("/auth.txt", server.arg("authother"));
      writeFile("/body.txt", server.arg("bodyother"));
      writeFile("/sensor_id.txt", server.arg("sensor_idother"));
         ...
         wifi_ssid = readFile("/ssid.txt");//server.arg("wifiname");
         wifi_pass = readFile("/pass.txt"); //server.arg("wifipassword");

         Serial.println("WiFi Name received: " + wifi_ssid);
         Serial.println("Stored in SPIFFS");
         ...
         if (wifi_ssid.length() > 0) {
            ...
            // Try connecting to user's WiFi
               ...
               message  = "<h2>Merci ! Votre Watty a pu se connecter au Wifi.</h2>";
                message += "<p>Vous pouvez desormais debrancher le cable USB et brancher le Watty  sur votre compteur Linky, comme indique dans la procedure.</p>";

            //   if (could_connect) {
            //   message += "<p>Connection au serveur confirmee !! Vous pouvez desormais debrancher le cable USB et brancher le dongle Linky sur votre compteur, comme indique dans la procedure.</p>";

           //    } else {
           //    message += "<h2> Cependant, le dongle n'a pas pu se connecter au serveur... </h2> </br>";
            //   message += "<p> Veuillez revenir sur la page precedente et re-selectionnez le wifi, mot de passe, et n oubliez pas de choisir un fournisseur de service (serveur) du dongle.</p> </br>";
           //    }
...
String HTML()
{
    msg="<h1>Configurez votre Watty</h1>\n";

   msg += "<form action='/save' method='post'>";

   msg += "<h3>WiFi</h3>";
   msg+="<h4>Choisissez votre Wifi, et entrez le mot de passe de votre box internet/de votre wifi.</h4>\n";
   msg+="<p style=\"text-align: center; margin: 0 10%; font-size: 0.7em;  color: #888; \">"
  "Si votre reseau Wifi ne s'affiche pas : <br> Rapprochez-vous de la source du Wifi et re-actualisez cette page. <br> " 
  "S'il n'apparait toujours pas, resettez votre Watty (bouton sur le cote), puis reconnectez vous dessus.  <br> "
  "Enfin, redemarrez votre box internet.<br>  <br> </p> \n";

       msg += "<label for='ssid'>Reseau WiFi:</label><br>\n";
    msg += "<select name='ssid' id='ssid'>\n";
...
//  msg += "SSID: <input name='ssid'><br>";
    msg+="<label for='pass'>Mot de passe du WiFi (Mot de passe de votre box internet):</label><br>\n";
        msg+="<input type='text' id='pass' name='pass'><br><br>\n";

//  msg += "Password: <input name='pass' type='password'><br><br>";

  msg += "<h3>Service Provider</h3>";
  msg += "<select name='provider' onchange='showProviderFields(this.value)'>";
  msg += "<option value='none'> </option>";
  msg += "<option value='unica'>UniCA IMREDD</option>";
  msg += "<option value='chargeangels'>Charge Angels</option>";
  msg += "<option value='other'>Other</option>";
  msg += "</select><br><br>";

    // UniCA block
  msg += "<div id='unica_fields' style='display:none'>";
  msg += "Sensor ID: <input name='sensor_idunica' value='LinkyHouse4'><br>";
  msg += "</div>";

    // Charge Angels block
  msg += "<div id='chargeangels_fields' style='display:none'>";
    msg += "Host: <input name='hostca'><br>";
  msg += "Asset ID: <input name='asset_idca'><br>";
  msg += "URL: <input name='urlca'><br>";
  msg += "Bearer Token:<br><textarea name='tokenca' rows='6' cols='60'></textarea><br>";
  msg += "</div>";

    // Other block
  msg += "<div id='other_fields' style='display:none'>";
  msg += "Host: <input name='hostother'><br>";
  msg += "URL: <input name='urlother'><br>";
  msg += "Port: <input name='portother'><br>";
   msg += "Sensor ID: <input name='sensor_idother' value='LinkyHouse4'><br>";

  msg += "Method: <select name='methodother'><option>GET</option><option>POST</option><option>PATCH</option></select><br>";
  msg += "Content-Type: <input name='ctypeother' value='application/json'><br>";
  msg += "Authorization: <input name='authother' rows='6' cols='60'></textarea><br>";
  msg += "Custom Body:<br><textarea name='bodyother' rows='6' cols='60'></textarea><br>";
  //  msg += "Bearer Token:<br><textarea name='tokenother' rows='6' cols='60'></textarea><br>";

  msg += "</div>";
  msg+="<input type='submit' value='Valider'>\n";
//  msg += "<br><input type='submit' value='Save'>";
  msg += "</form>";
      msg+="<p>Lorsque vous cliquez sur \"Valider\" votre Watty essaiera de se connecter au reseau choisi avec le mot de passe. Pour reconfigurer, rebranchez le Watty.</p>\n";
...
Serial.println(prov);
  ...
  client.setInsecure(); //the magic line, use with caution
    ...
    if (client.connect(host_url.c_str(), 443)) {
      ...
      Serial.println("Connection to charge Angels failed :()");
      return false;
}
