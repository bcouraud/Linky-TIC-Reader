...
#define STASSID "VM9478242" //"Livebox-CB4A" //"VM9478242" // "Livebox-CB4A" //
#define STAPSK "xbeubowf6ceNiJjx" //"FZNdu4aWShWEp3NSox" //"xbeubowf6ceNiJjx" // "FZNdu4aWShWEp3NSox" //
...
int Powermax = 19000; // to be read from Linky, and stored in EEPROM
long int t1 = 0;
long int t2=0;
...
float mapVoltageToSleep(float voltage) {
  const float Vmax = 6.05;
  const float Vmin = 5.5;
  const float Tmin = 15.0; // seconds
  const float Tmax = 50.0; // seconds
...
client.stop();
    if (client.connect(github_host, 443)) {
     ...
     // String serverPath = serverName+String(rawADC)+"&power="+String(puissanceCommuniquee)+"&counter="+String(counter_moyenne)+"&danger="+String(Danger)+"&wakeupnumber="+counterwakeup;// + "?temperature=24.37";
...
Serial.println("End of Reading loop");
  ...
  //       if(rawADC>voltageThreshold && finished_recording) {  // if we have enough voltage, we send the data and reinitialise the values real_voltage = (rawADC/ 1024) * 2.97 * 10;  //5.24V --> 181  or 5.06V -->174  or 4.90 -->170 or 4.18 --> 147
         if((debug==1 && counter_moyenne>=1) || (real_voltage>voltageThreshold && finished_recording && counter_moyenne>=1)){ // && !dataEEPROM.sent_last_time)) {  // if we have enough voltage, we send the data and reinitialise the values
