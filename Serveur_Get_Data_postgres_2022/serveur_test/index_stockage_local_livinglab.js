...
// in terminal:
// npm i express
// node index_Local_CSV.js

// then in browser:
// http://<IPaddress>>:3000/sendData?power=5&name=puissance
...
// server.js
//requires in terminal: npm i express
...
// http://192.168.1.95:3000/sendData?power=5&name=puissance
app.get('/sendData', function(req, res) {
  var newpower = req.query.power;
  var name = req.query.name;
  var newvoltage = req.query.voltage;
  var newdanger = req.query.danger;
  var newcounter = req.query.counter;
  console.log('power:'+newpower);
  newpower = newpower/newcounter;
  var newwakeupnb = req.query.wakeupnumber;
  // clientAtlas.connect();
  var now = new Date();
...
//store in local csv
var now = new Date();
var data = "\r\n"+ now.toLocaleString()+"," + newpower +","+ newvoltage +"," + Math.floor(Date.now() / 1000)+","+newdanger+","+newcounter+","+newwakeupnb;
...
var newPowerLimit = 500;
// res.status(200).send('Thanks ' +newpower + ' and name = ' +name);
res.status(200).send('newPower Limit = '+newPowerLimit);
});
